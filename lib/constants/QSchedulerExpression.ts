...
function checkTime(time: Time): void {
...
export function EVERY_FIRST_DAY_OF_MONTH_AT(time: Time): string {
  checkTime(time);
  return `${time.minute} ${time.hour} 1 * *`;
}

export function EVERY_DAY_AT(time: Time): string {
  checkTime(time);
  return `${time.minute} ${time.hour} * * *`;
}

export function EVERY_WEEK_AT(time: Time, day: WEEK_DAYS): string {
  checkTime(time);
